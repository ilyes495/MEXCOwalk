...
models = [
    ...
    # "mutex_t05_ncomb_cov", "mutex_t05_nsep_cov", "mutex_t06_ncomb_cov", "mutex_t06_nsep_cov", \
    ...
    "mutex_t09_ncomb_cov_nsep", "mutex_t09_nsep_cov_ncomb", #"mutex_t10_ncomb_cov", "mutex_t10_nsep_cov",\
...
#star construction by removing largest and getting isolated neighbors by outdegree
# def star_construction_bydegree (large_comp, largest_node_id):
#     all_neighbor_of_largest = set()
#     star_nodes_set = set([largest_node_id])
#
#     all_neighbor_of_largest = set(large_comp.in_edges(largest_node_id)).union(set(large_comp.out_edges(largest_node_id)))
#     print "\nall neignbors of largest\n", all_neighbor_of_largest
#     # for e in large_comp.edges:
#     #
#     #     if largest_node_id in e:
#     #         all_neighbor_of_largest.update(e)
...
def split_largest_comp(list_comp_graph, large_threshold = 0):
    ...
    lgq.append(largest_comp_pre)

    #finding large graph threshold
    max_out_degree_pre, largest_node_id_pre, largest_gene_id_pre = find_max_outdegree(largest_comp_pre)
    print "maxoutdegree", max_out_degree_pre
    if large_threshold == 0:
        threshold_lgq = max_out_degree_pre
    else:
        threshold_lgq = large_threshold

    ##################Testing both star_construction methods

    sc = set(star_construction(largest_comp_pre, largest_node_id_pre))
    #scd = set(star_construction_bydegree(largest_comp_pre, largest_node_id_pre))


    # print "\n\ndifference", sc.symmetric_difference(scd)
    # print "intersection", sc.intersection(scd), "\n\n"
    ##################

    # fn = path + "star_constr_n_" + str(len(sc)) + "_star_constr_d_" + str(len(scd))  + ".txt"
    #
    # with open(fn, "w") as f:
    #     f.write("star construction by neighbor\n"+ str(sc)+"\n")
    #     f.write("star construction by degree\n" + str(scd) + "\n")
    #     f.write("\n\ndifference\n" + str(sc.symmetric_difference(scd)) + "\n")
    #     f.write("\n\nintersection\n" + str(sc.intersection(scd)) + "\n")
    #     f.write("\n\ngenes in old (iso) but not in new function: \n" + str(list(scd-sc)) + "\n")
    #     f.close()


    #all cc from list graphs minus largest
    list_graph_leftover = list_comp_graph[:largest_comp_index_pre]+list_comp_graph[(largest_comp_index_pre+1):]

    while len(lgq) >0:
        print "\nnew element in large queue\n"
        largest_comp = lgq.popleft()
        ...
        for comp in scc:
            print "comp", comp
            if len(comp)> threshold_lgq:
                g = reduced_comps.subgraph(comp).copy()
                lgq.append(g)
                print "\nLGQ"
            elif len(comp)< threshold_small:
                g = reduced_comps.subgraph(comp).copy()
                small_components.append(g)
                print "\nsmall"
            else:
                g = reduced_comps.subgraph(comp).copy()
    ...
    for scm in range(len(main_comp_list)):
        ...
        if len(main_comp_list[scm]) <threshold_small:
            small_components.append(main_comp_list[scm])
    ...
    for scomp in range(len(small_components)):
        max_comp_score = 0
        max_comp_index = 0
        for c in range(len(main_comp_list)):
            comp_score = 0
            for gene_m in main_comp_list[c]:

                for gene_s in small_components[scomp]:
        ...
        print "\ncomponent", small_components[scomp].nodes, "maxcomp score+index", max_comp_score, max_comp_index
        print "list of comps nodes before", len (main_comp_list[max_comp_index]),main_comp_list[max_comp_index].nodes
        main_comp_list[max_comp_index].add_nodes_from(small_components[scomp])
    ...
    return list_graph_leftover[:]+main_comp_list[:]
...
for key in models:
    ...
    #input file path
    filepath = "../" + network_name + "/out/connected_components/" + key + "/"
    ...
    while num_start >= num_end:
        ...
        final_comp_list_graphs = split_largest_comp(component_list)
